...
export default function LeadForm({ defaultModel = '' }: { defaultModel?: string }) {
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      await emailjs.send(EJS_SERVICE, EJS_TEMPLATE, {
        ...
        produkt:    'ZAPYTANIE — Pionowe maszyny pakujące VFFS (Essegi)',
      }, EJS_KEY);
      setState('success');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (window as any).gtag?.('event', 'generate_lead', { event_category: 'vffs_inquiry' });
      setForm(EMPTY);
    } catch {
      setState('error');
    }
  ...
  return (
    <Box id="zapytaj" component="section" sx={{ bgcolor: BG_ALT, borderBottom: `1px solid ${BORDER}`, py: { xs: 10, md: 14 } }}>
        ...
        <Box sx={{
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {[
                'Dobór modelu i systemu dozowania do produktu',
          ...
          <Box component="form" onSubmit={submit} noValidate sx={{
            ...
            {state === 'success' && <Alert severity="success">Dziękujemy! Odezwiemy się w ciągu 1 dnia roboczego.</Alert>}
            {state === 'error' && <Alert severity="error">Wystąpił błąd. Spróbuj ponownie lub zadzwoń: +48 123 454 397.</Alert>}

            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
              <TextField fullWidth label="Imię i nazwisko *"
              ...
              <TextField fullWidth label="Firma"
                value={form.company} onChange={e => set('company', e.target.value)}
                sx={{ ...fieldSx, gridColumn: '1 / -1' }} />
            </Box>

            <TextField select fullWidth label="Model, który Cię interesuje"
              value={form.model} onChange={e => set('model', e.target.value)} sx={fieldSx}>
              {MODELS_LIST.map(m => <MenuItem key={m} value={m}>{m}</MenuItem>)}
            </TextField>

            <TextField fullWidth label="Dodatkowe informacje" multiline rows={3}
              value={form.message} onChange={e => set('message', e.target.value)} sx={fieldSx} />
            ...
            />
            {errors.consent && (
              <Typography sx={{ fontSize: '0.75rem', color: 'error.main', mt: -1.5 }}>{errors.consent}</Typography>
            )}

            <Button type="submit" variant="contained" disabled={state === 'loading'} disableElevation
              ...
              {state === 'loading' ? <CircularProgress size={22} sx={{ color: '#fff' }} /> : 'Wyślij zapytanie'}
            </Button>
          </Box>
        </Box>
      </Container>
